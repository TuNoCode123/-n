...
const emit = defineEmits(["updateListAttribute"]);
const selected = ref(toRaw(selectedImage.value));
if (initState.value == false) {
  emit("updateListAttribute", null, props.uuid, "image", selected.value.image);
...
watch(
  () => selected.value,
  (n, o) => {
    emit("updateListAttribute", null, props.uuid, "image", n.image);
  },
  {
    deep: true,
  }
